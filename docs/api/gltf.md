...
texture coordinates, and unsigned-short indices. `ExportGltfFileAsync` emits
the same projection as a JSON manifest plus a content-addressed `.bin`
sidecar. It commits the manifest last, so a committed package never references
missing or partial output. A failed operation may leave an unreferenced
...
fingerprints before restoring serialized MSH state. Applicable metadata
...
flags. A successful import retains the asset lineage and rotates the document
identity. Edited or unsupported content fails closed and returns no partial
asset.
...
positions and normals, triangle topology, valid indices, required texture
coordinates, and `GltfOperationProfile.MaxActiveRenderVertices`.
